...
def process_image(image, background_type):
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # Apply binary thresholding based on background type
    ...
    for i, contour in enumerate(filtered_contours):
        ...
        if len(contour) >= 5:
            ...
            cv2.ellipse(output_image, ellipse, ellipse_color, 15)
...
background_type = st.selectbox("Select background type:", ["White", "Black"])
...
if uploaded_file is not None:
    ...
    if image is None:
        ...
        # Process the image
        output_image, rice_grain_dimensions, total_grain_count, whole_grain_count, broken_grain_count, normal_grain_count = process_image(image, background_type)
